...
class RegisterUserUseCase(
    ...
    suspend fun execute(command: RegisterUserCommand): AuthResult {
        ...
        eventPublisher.publish(
            stream = "users",
            fields = mapOf(
                "event" to "user.registered",
                "userId" to savedUser.id.value.toString(),
                "login" to savedUser.login.value,
                "portfolioId" to savedUser.portfolioId.value.toString(),
            ),
        )
        telemetryRecorder.record(
            event = "user.registered",
            attributes = mapOf(
                "user.id" to savedUser.id.value.toString(),
                "portfolio.id" to savedUser.portfolioId.value.toString(),
            ),
        )
