...
\newcommand{\projectscreenshot}[2]{%
    \IfFileExists{../../docs/images/#1.png}{%
        \includegraphics[width=\linewidth,height=0.33\textheight,keepaspectratio]{../../docs/images/#1.png}%
    }{%
        \IfFileExists{../../docs/images/#1.jpg}{%
            \includegraphics[width=\linewidth,height=0.33\textheight,keepaspectratio]{../../docs/images/#1.jpg}%
        }{%
            \IfFileExists{../../docs/images/#1.jpeg}{%
                \includegraphics[width=\linewidth,height=0.33\textheight,keepaspectratio]{../../docs/images/#1.jpeg}%
            }{%
                \fbox{\parbox{0.9\linewidth}{\centering Скриншот не найден по пути \texttt{docs/images/#1.(png|jpg|jpeg)}}}%
            }%
        }%
    }%
}
...
Отчёт подготовлен по актуальному состоянию репозитория после обновления ветки \texttt{main} на 8 апреля 2026 года и повторного импорта Android-клиента из внешнего репозитория команды. Старые feature-ветки общего репозитория рассмотрены как этапы развития backend-части, а архитектурный baseline определяется именно актуальным \texttt{main}. В свежей версии backend дополнительно появились проверки денежного баланса, сценарий пополнения портфеля и уточнённая обработка отсутствующих символов в holdings. Структура отчёта соответствует требованиям лекционных материалов курса: техническое задание, описание архитектуры, описание реализации и описание системы тестирования~[1].
...
\begin{enumerate}[leftmargin=1.2cm]
    ...
    \item Пополнение денежного баланса пользовательского портфеля.
...
\begin{figure}[htbp]
    ...
    \caption{Архитектура клиент-серверной системы проекта}
\end{figure}

На рисунке показана целевая backend-схема проекта, зафиксированная в документации и текущей структуре репозитория. Пользовательский сценарий начинается в Android-клиенте, проходит через внешний gateway \texttt{mobile-api} и попадает во внутренний транзакционный сервис \texttt{mobile-backend}. Отдельно выделен контур рыночных данных: \texttt{quotes-service} получает котировки от Linux C-драйвера, публикует актуальные значения в Redis и сохраняет историю в ClickHouse. Также схема отражает observability-подсистему, где backend-сервисы экспортируют telemetry-данные в OpenTelemetry Collector и далее в стек мониторинга.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.96\textwidth]{../../docs/diagrams/mobile-flow-sequence.pdf}
    \caption{Последовательность взаимодействия мобильного клиента, gateway и backend}
\end{figure}

Эта схема показывает типовой пользовательский сценарий на уровне HTTP-взаимодействий между компонентами. Для входа пользователя \texttt{mobile-api} выступает только как внешний шлюз, а проверка учётных данных и выдача JWT выполняется во внутреннем сервисе \texttt{mobile-backend}. Для защищённых операций backend проверяет JWT, обращается к PostgreSQL за состоянием портфеля и, при необходимости, запрашивает актуальную цену у \texttt{quotes-service}. После выполнения покупки backend публикует событие \texttt{trade.executed} в Redis, а затем возвращает ответ обратно через gateway в мобильное приложение.
...
\subsection{Экраны мобильного интерфейса}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[t]{0.46\textwidth}
        \centering
        \projectscreenshot{profile}{Экран профиля пользователя}

        \small Экран профиля пользователя
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.46\textwidth}
        \centering
        \projectscreenshot{sber-stocks}{Экран карточки акции Сбербанка}

        \small Экран карточки акции Сбербанка
    \end{minipage}
    \caption{Экраны профиля пользователя и карточки акции}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[t]{0.46\textwidth}
        \centering
        \projectscreenshot{stocks-list}{Экран списка акций}

        \small Экран списка акций
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.46\textwidth}
        \centering
        \projectscreenshot{portfilio_statistics}{Экран статистики портфеля}

        \small Экран статистики портфеля
    \end{minipage}
    \caption{Экраны списка акций и статистики портфеля}
\end{figure}
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item use case пополнения денежного баланса;
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item покупки акций и проверок денежного баланса;
...
\begin{enumerate}[leftmargin=1.2cm]
    ...
    \item пополнение денежного баланса;
...
В качестве отдельного примера системной проверки был выполнен нагрузочный прогон публичного endpoint получения котировки. Использовался сценарий на \(10000\) запросов с уровнем параллелизма \(100\) к endpoint \texttt{https://song-analysis.app/market/quotes/AAPL}.

Зафиксированные результаты:

\begin{itemize}[leftmargin=1.2cm]
    \item общее число запросов --- \(10000\);
    \item успешных запросов --- \(10000\);
    \item неуспешных сетевых или необработанных запросов --- \(0\);
    \item общее время выполнения --- \(39239\) мс;
    \item производительность --- \(254.85\) запросов в секунду.
\end{itemize}

Этот результат нельзя считать исчерпывающим профилированием всей платформы, однако он полезен как подтверждение того, что внешний API-слой способен выдерживать интенсивную однотипную нагрузку без необработанных ошибок на зафиксированном эксперименте.
...
\bibitem{repo-docs}
Документация проекта \texttt{stock-tracker-app}: \texttt{README.md}, \texttt{docs/system-architecture.md}, \texttt{docs/backend-implementation.md}, \texttt{docs/endpoints.md}. Репозиторий команды, состояние на 08.04.2026.
