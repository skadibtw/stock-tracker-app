...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{../../docs/diagrams/system-architecture-4.jpg}
...
В качестве отдельного примера системной проверки была выполнена серия нагрузочных прогонов публичного endpoint получения котировки. Использовался сценарий на \(10000\) запросов с уровнем параллелизма \(100\) к endpoint \texttt{https://song-analysis.app/market/quotes/AAPL}. Всего было выполнено \(10\) запусков, то есть суммарно отправлено \(100000\) запросов.
...
\begin{itemize}[leftmargin=1.2cm]
    ...
    \item успешных запросов, включая обработанные приложением ошибки, --- \(9996\);
    \item неуспешных сетевых или необработанных запросов --- \(4\);
    \item общее время выполнения --- \(137273\) мс;
    \item производительность --- \(72.85\) запросов в секунду.
\end{itemize}
...
\begin{itemize}[leftmargin=1.2cm]
    \item общее число запросов --- \(10000\);
    \item успешных запросов, включая обработанные приложением ошибки, --- \(9852\);
    \item неуспешных сетевых или необработанных запросов --- \(148\);
    \item общее время выполнения --- \(108647\) мс;
    \item производительность --- \(92.04\) запросов в секунду.
\end{itemize}

По серии из \(10\) запусков средние показатели составили:

\begin{itemize}[leftmargin=1.2cm]
    \item суммарное число запросов --- \(100000\);
    \item суммарное число успешных запросов --- около \(99240\);
    \item суммарное число неуспешных сетевых или необработанных запросов --- около \(760\);
    \item среднее время одного прогона --- около \(123000\) мс;
    \item средняя производительность --- около \(82.45\) запросов в секунду.
\end{itemize}
