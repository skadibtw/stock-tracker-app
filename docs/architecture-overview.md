...
Этот документ фиксирует актуальную архитектуру проекта после обновления репозитория на 8 апреля 2026 года и повторного импорта Android-клиента в каталог `android-app/`.
...
| `mobile-backend` | Бизнес-логика, аутентификация, операции buy/sell, cash balance и статистика |
...
4. `mobile-backend` валидирует JWT, работает с PostgreSQL, контролирует денежный баланс и публикует события в Redis Streams.
...
На 8 апреля 2026 года Android-клиент ещё не интегрирован с backend-контуром:
...
- сценарий пополнения денежного баланса на клиенте пока не подключён к backend
...
- в backend уже добавлены проверки денежного баланса и отдельный маршрут пополнения портфеля
